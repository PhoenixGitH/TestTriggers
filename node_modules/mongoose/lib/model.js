 ...
 *
 *     API.sold = Date.now();
 *     API.save(function (err, API, numAffected) {
 *       if (err) ..
 *     })
 *
 ...
 *       API.findById(API._id, function (err, API) {
 ...
 *
 * ####Example:
 *     API.remove().then(function (API) {
 *        ...
 *     }).onRejected(function (err) {
 *        assert.ok(err)
 *     })
 *
 * @param {function(err,API)} [fn] optional callback
 * @return {Promise} Promise
 * @api public
 */
 ...
 *     })
 *
 * _NOTE: It is not recommended that you run this in APIion. Index creation may impact database performance depending on your load. Use with caution._
 *
